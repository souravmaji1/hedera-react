...
contract AirdropSystem is Ownable {
    ...
    event TokensClaimed(uint256 airdropId, address claimant, uint256 amount);
    event NftClaimed(uint256 airdropId, address claimant, uint256 nftId);
    ...
    function createERC20Airdrop(
        ...
        IERC20(_tokenAddress).transferFrom(msg.sender, address(this), _totalTokens);
    ...
    function createERC721Airdrop(
        ...
        require(_expirationTime > block.timestamp, "Expiration time must be in the future");
        require(_nftIds.length > 0, "At least one NFT ID must be provided");
        require(bytes(_title).length > 0, "Title is required");
    ...
    function _validatePercentages(
        ...
        require(_first > 0, "First claim percentage must be greater than 0");
        require(_second > 0, "Second claim percentage must be greater than 0");
        require(_other > 0, "Other claim percentage must be greater than 0");
        require(
            _first + _second + _other <= 100,
    ...
    function addToWhitelist(uint256 _airdropId, address[] memory _users) external {
        ...
        require(msg.sender == airdrop.basic.creator, "Only the airdrop creator can add to the whitelist");
    ...
    function claimTokens(uint256 _airdropId) external {
        Airdrop storage airdrop = airdrops[_airdropId];
        require(airdrop.basic.isActive, "Airdrop is not active");
        require(block.timestamp <= airdrop.basic.expirationTime, "Airdrop has expired");
        require(!airdrop.hasClaimed[msg.sender], "You have already claimed");
        ...
        for (uint256 i = 0; i < airdrop.conditions.length; i++) {
            ...
            if (condition.conditionTokenAddress != address(0)) {
                ...
                require(
                    ...
                    "You do not meet the token holding condition"
                );
            }
        }
    }
    ...
    function _claimERC20(Airdrop storage airdrop, uint256 airdropId) private {
        ...
        IERC20(airdrop.basic.tokenAddress).transfer(msg.sender, claimableAmount);
    ...
    function _claimERC721(Airdrop storage airdrop, uint256 airdropId) private {
        ...
        for (uint256 i = 0; i < airdrop.basic.nftIds.length; i++) {
            if (!airdrop.claimedNfts[airdrop.basic.nftIds[i]]) {
                nftIdToClaim = airdrop.basic.nftIds[i];
                found = true;
                break;
            }
        }
        require(found, "No NFTs available to claim");
    ...
    function _getAvailableNftIds(Airdrop storage airdrop) private view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < airdrop.basic.nftIds.length; i++) {
            if (!airdrop.claimedNfts[airdrop.basic.nftIds[i]]) {
                availableNftIds[nftIndex] = airdrop.basic.nftIds[i];
                nftIndex++;
            }
        }
    ...
    function removeFromWhitelist(uint256 _airdropId, address[] memory _users) external {
        Airdrop storage airdrop = airdrops[_airdropId];
        require(msg.sender == airdrop.basic.creator, "Only the airdrop creator can remove from the whitelist");
    ...
    function withdrawRemainingTokens(uint256 _airdropId) external onlyOwner {
        Airdrop storage airdrop = airdrops[_airdropId];
        require(!airdrop.basic.isActive || block.timestamp > airdrop.basic.expirationTime, "Airdrop is still active");

        if (airdrop.basic.tokenType == TokenType.ERC20) {
            uint256 remainingTokens = airdrop.basic.totalTokens - airdrop.basic.claimedTokens;
            require(remainingTokens > 0, "No tokens remaining");
            IERC20(airdrop.basic.tokenAddress).transfer(owner(), remainingTokens);
        } else {
            for (uint256 i = 0; i < airdrop.basic.nftIds.length; i++) {
                if (!airdrop.claimedNfts[airdrop.basic.nftIds[i]]) {
                    IERC721(airdrop.basic.tokenAddress).transferFrom(address(this), owner(), airdrop.basic.nftIds[i]);
                    airdrop.claimedNfts[airdrop.basic.nftIds[i]] = true;
                }
            }
        }
    }
